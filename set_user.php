...
error_reporting(E_ALL);
	...
	$users = array();
	$user_names = array();
	$sql_users = "SELECT user.id, user.user_name, user.user_created, ugroup.group_id, 
    ugroup.group_name, employee.empl_id, employee.empl_name 
	FROM user LEFT JOIN ugroup ON ugroup.group_id = user.ugroup_id LEFT JOIN employee ON user.empl_id = employee.empl_id 
    WHERE user.id != 0 
	ORDER BY user_name";
	...
	$sql_ugroup = "SELECT group_id, group_name FROM ugroup";
	...
	if(isset($_POST["save_changes"])){
		...
		$user_pw = password_hash((sanitize($db_link, $_POST['user_pw'])), PASSWORD_DEFAULT);
		...
		if($user_id == 0){
			...
			$sql_user_upd = "UPDATE user SET user_name = '$user_name', user_pw = '$user_pw', ugroup_id = $ugroup, 
            empl_id = $empl_id, user_created = $timestamp WHERE id = $user_id";
							...
							<td class="center">
								<select name="ugroup" size="1" <?PHP if ($user_id == 1) echo ' disabled="disabled"'; ?> >
									...
									while ($row_ugroup = mysqli_fetch_assoc($query_ugroup)){
											...
											echo '>'.$row_ugroup['group_name'].'</option>';
										}
									?>
								</select>
							</td>
